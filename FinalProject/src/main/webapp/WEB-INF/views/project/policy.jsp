		...
		<div class="col-lg-8">
			<div class="blog-details__main">
				<div class="meta">
					...
					<small> 펀딩 종료 후 후원자의 불만 또는 분쟁 발생 시 중요한 기준이 될 수 있습니다. 신중하게 작성해주세요 </small>
					<div class="blog-author__content" style="padding-top: 1em;">
						<ul style="text-align: left; display: inline-block; line-height: 2em;">
							<li><i class="flaticon-checkmark"></i> 펀딩 마감 이후에는 후원금액이 창작자에게 전달되기에 렛어스를 통해 환불이 불가합니다.</li>
							<li><i class="flaticon-checkmark"></i> 환불은 창작자가 환불계좌를
							받아 반환하는 방식으로만 가능하며, 어떤 경우에 환불이 가능한지를 기재해주세요.</li>
							<li><i class="flaticon-checkmark"></i> 펀딩 성사 후
							지연 또는 선물 전달에 문제가 발생했을 때에 어떤 식으로 대응하시겠어요? 이 프로젝트 특성상 발생할 수 있는 문제에
							대한 추가적인 정책이나 약속이 있다면 기재해주세요.</li>
							<li><i class="flaticon-checkmark"></i> 배송이
							필요한 선물에 문제가 발생한 경우 교환, A/S는 어떻게 진행되나요?</li>
							<li><i class="flaticon-checkmark"></i> 오프라인으로 전달되는 선물(공연, 전시 등)은 타인에게 양도가
							가능한가요?</li>
						</ul>
					</div>
				</div>
				...
				<script>
					var editor3 = new toastui.Editor({
						...
						hooks : { // 사진 인식 hook은 필요할 때 자기 알아서 나옴,,,,
							addImageBlobHook : function imageConverter(blob,
									callback) {
				...
				<div class="text-center">
					<button type="button" class="main-btn" onclick="updatePolicy();" style="background-color: #A4AC85; border: none;">다음</button>
				</div>
			</div>
		</div>
	</div>
